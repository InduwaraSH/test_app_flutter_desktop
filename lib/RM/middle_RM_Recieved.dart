...
class middle_RM_Recived_new extends StatefulWidget {
  final String location;
  final String position;
  const middle_RM_Recived_new({
    super.key,
  ...
  @override
  State<middle_RM_Recived_new> createState() => _homeState();
}

class _homeState extends State<middle_RM_Recived_new> {
    ...
    if (snapshot.exists) {
      ...
      snapshot.children.forEach((child) {
        final data = Map<String, dynamic>.from(
          child.child("timberReportheadlines").value as Map,
        );
        data['key'] = child.key;
        temp.add(data);
      });
  ...
  Widget SENDItem({required Map Alerts, required int index}) {
    return _AnimatedSendCard(
      alerts: Alerts,
      isActive: selectedBranch == Alerts['serialnum'],
      onSelect: () {
        setState(() {
        ...
        rmSent.setSNum(Alerts['serialnum'].toString());
        rmSent.setPOC(Alerts['serialnum'].toString());
          ...
          child: AnimatedContainer(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  "Serial Number: ${widget.alerts['serialnum'] ?? 'N/A'}",
                  style: TextStyle(
                    fontSize: 14,
                    fontFamily: 'sfproRoundSemiB',
